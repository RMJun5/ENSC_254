...
Instruction parse_instruction(uint32_t instruction_bits) {
  ...
  // Bitmask - Binary pattern used to select, modify, or test specific bits within a data value using bitwise operations.
  instruction.opcode = instruction_bits & 0x7F; // Extract opcode (bits 0-6)

  switch (instruction.opcode) {
  case 0x33: // R-Type instruction (e.g. add, sub, mul)
    instruction.rtype.rd = (instruction_bits >> 7) & 0x1F; // Extract rd (bits 7-11) (destination register)
    instruction.rtype.funct3 = (instruction_bits >> 12) & 0x7; // Extract funct3 (bits 12-14)
    instruction.rtype.rs1 = (instruction_bits >> 15) & 0x1F; // Extract rs1 (bits 15-19) (source registers)
    instruction.rtype.rs2 = (instruction_bits >> 20) & 0x1F; // Extract rs2 (bits 20-24) (source registers)
  ...
  case 0x37: // U-Type (Load Upper Immediate)
  // Loads the highest 16 bits of the registor with a constant then clears the rest with 0
    ...
    break;
  case 0x6F: // UJ-Type (Jump And Link)
  // Saves the address of the next instruction into a register and then jumps to a target address, enabling function calls.
  case 0x73: // I-Type (ECALL - System Call)
  // Triggers a system call, allowing a program to request a service from the operating system or simulator
