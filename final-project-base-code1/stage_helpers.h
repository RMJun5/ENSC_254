...
uint32_t gen_alu_control(idex_reg_t idex_reg){
  ...
  switch(idex_reg.aluop){ // Determine ALU operation type based on aluop field
    ...
    default: // If aluop does not match any case
      break;
  }
...
idex_reg_t gen_control(Instruction instr)
{
  ...
  switch (instr.opcode) {
    ...
    case 0x33: // R-type instructions
      ...
      idex_reg.aluop = 10;     // ALU operation code for R-type
      break;
    ...
    case 0x3: // I-type load instructions
      ...
      idex_reg.mem_write = 0;  // No memory write
      idex_reg.branch = 0;     // No branch
      idex_reg.aluop = 0;      // ALU op for load/store/branch/jump
      break;
    ...
    case 0x13: // I-type arithmetic instructions
      ...
      idex_reg.mem_write = 0;  // No memory write
      idex_reg.branch = 0;     // No branch
      idex_reg.aluop = 1;      // ALU op code for I-type arithmetic (fixed 1, not octal)
      break;
    ...
    case 0x37: // U-type (LUI) instructions
      ...
      idex_reg.mem_write = 0;  // No memory write
      idex_reg.branch = 0;     // No branch
      idex_reg.aluop = 11;     // ALU op code for U-type
      break;
